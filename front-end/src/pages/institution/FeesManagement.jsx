  ...
  return (
    <div className="fees-page">
      ...
      <section className="fees-panel fees-table-panel">
        ...
        <div className="fees-table-scroll"><table><thead><tr><th>Student</th><th>Student Code</th><th>Fee Type</th><th>Period</th><th>Amount</th><th>Status</th><th>Remaining</th></tr></thead><tbody>{filteredFees.map((fee) => <tr key={fee.id}><td><strong>{fee.student}</strong></td><td>{fee.studentCode}</td><td>{fee.feeType}</td><td>{fee.period}</td><td>{money(fee.amount, fee.currency)}</td><td><span className={`fees-status fees-status-${fee.status}`}>{statusLabel(fee.status)}</span></td><td>{money(fee.remaining, fee.currency)}</td></tr>)}</tbody></table>{filteredFees.length === 0 && <div className="fees-empty"><strong>No fee records have been added yet</strong><span>New student fee records will appear here.</span></div>}</div>
      </section>
